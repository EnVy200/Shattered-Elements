...
namespace TrueSoul.Enemies
{
	...
	public abstract class WormHead : Worm
	{
		...
		/// <summary>
		/// The distance the segments are expected to be apart, including the head and tail segments
		/// </summary>
		public int SegmentSeperation { get; set; }
		...
		protected int SpawnSegment(IEntitySource source, int type, int latestNPC)
		{
			...
			Main.npc[latestNPC].ai[3] = SegmentSeperation;
	...
	public abstract class WormBody : Worm
	{
		...
		internal static void CommonAI_BodyTail(Worm worm)
		{
			...
			if (following is not null)
			{
				...
				float dist = (length - worm.NPC.ai[3]) / length;
